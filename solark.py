...
class SolarkConnector:
    def __init__(self, solark_conf: dict) -> None:
        ...
        if self.max_days <= 0:
            raise Exception(
                f"Invalid max_days {self.max_days}. Must be a positive integer."
            )
        self.client = SolArkClient(username=self.username, password=self.password)
        self.client.login()
        self.plant = self.client.get_plant(plant_id)
        logging.info(
            f"Initialized SolarkConnector for plant {self.plant.name} (ID: {self.plant.plant_id})"
        )

    def __get_data(
        self, day: str, measurement: str, retry: bool
    ) -> SolArkSeriesCollection:
        try:
            if measurement == "plant_energy":
                return self.client.get_plant_energy(
                    self.plant.plant_id, period="day", date=day
                )
            if measurement == "plant_power":
                return self.client.get_plant_power(
                    self.plant.plant_id, period="day", date=day
                )
        ...
        raise Exception(
            f"Invalid measurement {measurement}. Must be either 'plant_energy' or 'plant_power'."
        )
    ...
    def get_data(self, time_utc: datetime, measurement: str) -> list[dict]:
        try:
            ...
            if plant_timezone and plant_timezone.code:
                day_local = (
                    time_utc.replace(tzinfo=ZoneInfo("UTC"))
                    .astimezone(ZoneInfo(plant_timezone.code))
                    .date()
                )
            else:
                logging.warning(
                    f"Plant timezone not set for plant {self.plant.plant_id}. Assuming UTC."
                )
                day_local = time_utc.date()
            logging.debug(
                f"Converted UTC datetime {time_utc} to local date {day_local}"
            )
            day_str = day_local.strftime("%Y-%m-%d")

            logging.debug(f"Processing data for {day_str}, measurement: {measurement}")
            series_collection: SolArkSeriesCollection = self.__get_data(
                day_str, measurement, True
            )
            if not series_collection:
                logging.error(
                    f"No data returned from Sol-Ark for {day_str} using {measurement}."
                )
                return []
            ...
            for s in series_collection.series:
                if not s.label:
                    logging.warning(
                        f"Series label is missing for series with unit {s.unit}!"
                    )
                    continue
                for r in s.records:
                    ...
                    if not result:
                        ...
                        logging.debug(
                            f"Converting local time {r.time} to UTC for {day_str}"
                        )
                        if plant_timezone and plant_timezone.code:
                            result_datetime = result_datetime.replace(
                                tzinfo=ZoneInfo(plant_timezone.code)
                            )
                            result_datetime = result_datetime.astimezone(
                                ZoneInfo("UTC")
                            )
