...
class InfluxConnector:
    def __init__(self, influx_conf: dict):
        ...
        logging.debug(
            f"Influx conf: url={self.url};bucket={self.bucket};org={self.org};no-op={self.no_op}"
        )

    def get_last_recorded_time_utc(
        self, max_days: int, to_time: datetime, measurement: str
    ) -> datetime:
        if self.no_op:
        ...
        if len(results) == 0:
            logging.info(
                f"Found no records dated less than {max_days} days(s) in influx bucket {self.bucket} measurement {measurement}."
            )
